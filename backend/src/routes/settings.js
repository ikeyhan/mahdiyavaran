...
const PUBLIC_KEYS = ['site_name', 'site_title', 'domain', 'maintenance', 'registration', 'online_payment', 'contact_email', 'contact_phone', 'address', 'social_instagram', 'social_telegram', 'social_whatsapp', 'social_linkedin', 'site_description', 'shipping_cost', 'free_shipping_min', 'min_order'];
...
router.get('/', (req, res) => {
  ...
  // کلید API هوش مصنوعی هرگز به مرورگر برگردانده نمی‌شود
  obj.ai_api_key_set = obj.ai_api_key ? '1' : '0';
  obj.ai_api_key = '';
  res.json({ settings: obj });
});
...
router.put('/', requireRole('admin'), (req, res) => {
  ...
  const tx = db.transaction(() => { for (const [k, v] of entries) { if (k === 'ai_api_key_set') continue; if (k === 'ai_api_key' && !String(v || '').trim()) continue; up.run(str(k, 60), str(v, 2000)); } });
  tx();
